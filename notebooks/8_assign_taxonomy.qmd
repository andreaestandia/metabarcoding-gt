...
blast <- read.table(file.path(LOCUS_DIR, "outputs",
                              "blast",
                              "blast_results_A11_ASVs.tab"), header = FALSE)
...
get_genus_species_filtered <- function(acc, target_region = "europe") {
  tryCatch({
    summary <- entrez_summary(db = "nuccore", id = acc)
    title <- summary$title
    ...
    # Keywords and common non-European regions/countries
    exclude_keywords <- c(
    ...
    # Check if any excluded keyword appears in the title
    geographic_filter <- !any(str_detect(title, regex(paste(exclude_keywords, collapse = "|"), ignore_case = TRUE)))
    
    if (!geographic_filter) {
      return(c(species = NA, genus = NA, filter_reason = "geographic"))
    }
    
    return(c(species = species, genus = genus, filter_reason = "passed"))
    
  }, error = function(e) {
    return(c(species = NA, genus = NA, filter_reason = "error"))
  })
}
...
for (file in tab_files) {
  ...
  tax_info <- t(sapply(unique_accessions, get_genus_species_filtered))
...
for (file in tab_files) {
  ...
  blast <- read.table(file, header = FALSE, stringsAsFactors = FALSE) %>%
    mutate(accession = str_extract(V2, "(?<=\\|)[A-Z]{1,2}_?\\d+\\.\\d+"))
  ...
  # Merge tax info with original BLAST matches
  blast_matches <- left_join(blast, tax_df, by = "accession") %>%
    select(query = V1, accession, genus, species, filter_reason)
  
  # Save as CSV per file
  out_file <- file.path(output_dir,
                        paste0(tools::file_path_sans_ext(basename(file)), "_matches.csv"))
  write_csv(blast_matches, out_file)
}
