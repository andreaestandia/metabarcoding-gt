...
LOCUS <- readRDS(file.path(LOCUS_DIR, "R_objects", "locus.rds"))
...
saveRDS(fnFs.filtN, file = file.path(LOCUS_DIR, "R_objects", "02_fnFs.filtN.rds"))
saveRDS(fnRs.filtN, file = file.path(LOCUS_DIR, "R_objects", "02_fnRs.filtN.rds"))
...
saveRDS(fnFs.cut, file.path(LOCUS_DIR, "R_objects", "02_fnFs.cut.rds"))
saveRDS(fnRs.cut, file.path(LOCUS_DIR, "R_objects", "02_fnRs.cut.rds"))
...
cutadapt_path <- "/home/estandia/miniconda3/envs/cutadapt38/bin/cutadapt"

cutadapt_installed <- file.exists(cutadapt_path)
if (!cutadapt_installed) {
  cat("WARNING: cutadapt not found at", cutadapt_path, "\n")
} else {
  for (i in seq_along(fnFs.filtN)) {
    cmd <- paste(cutadapt_path,
                 R1.flags, R2.flags,
                 "-o", fnFs.cut[i],
                 "-p", fnRs.cut[i],
